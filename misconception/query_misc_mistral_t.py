...
def call_mistral_api(history, prompt, model="mistral-large-latest", repeat: int = 1, retries: int = 6):
    try:
        ...
        if retries==0:
            pass
        sleep(2**(8-retries))
        return call_mistral_api(history, prompt, repeat, retries-1)
    ...
    for d in tqdm(data):
        ...
        while res[0].upper() not in ["A","B","C","D"]:
            if repeats==0:
                ...
                break
            repeats-=1
            res=call_mistral_api(history, system_prompt, repeat=1)
        d["Mistral_t"]={"answer":res[0].upper(), "correct":answer, "reasoning":cot}

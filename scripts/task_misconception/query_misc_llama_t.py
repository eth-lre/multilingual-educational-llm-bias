...
def call_together_api(history, prompt, model="meta-llama/Meta-Llama-3.1-405B-Instruct-Turbo", repeat: int = 1, retries: int = 6):
    try:
        ...
        if retries==0:
            pass
        sleep(2**(8-retries))
        return call_together_api(history, prompt, repeat, retries-1)
    ...
    for d in tqdm(data):
        ...
        while res[0].upper() not in ["A","B","C","D"]:
            if repeats==0:
                ...
                break
            repeats-=1
            res=call_together_api(history, system_prompt, repeat=1)
        d["Llama"]={"answer":res[0].upper(), "correct":answer, "reasoning":cot}

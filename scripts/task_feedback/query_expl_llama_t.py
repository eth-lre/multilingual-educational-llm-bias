...
client = Together(api_key=keys.together)
def call_together_api(history, prompt, model="meta-llama/Meta-Llama-3.1-405B-Instruct-Turbo", repeat: int = 1, retries: int = 6):
    try:
        ...
        if retries==0:
            return "E"
        sleep(2**(8-retries))
        return call_together_api(history, prompt, model, repeat, retries-1)
...
if __name__=="__main__":
    ...
    for d in tqdm(data):
        ...
        cot=call_together_api(history, system_prompt, repeat=1)
        ...
        while res[0].upper() not in ["A","B","C","D"]:
            if repeats==0:
                ...
                break
            repeats-=1
            res=call_together_api(history, second_prompt, repeat=1)
        d["Llamae"]={"answer":res[0].upper(), "correct":answer, "reasoning":cot}
